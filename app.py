...
# ----------------------------------------------------
# 5. Fluxo de Autenticação Seguro (AvJuris IA - Redirecionamento na Mesma Aba)
# ----------------------------------------------------
...
if not st.session_state.user_session:
    # Captura o token vindo do retorno do Google
    components.html("""
        <script>
        try {
...
.auth-divider { display: flex; align-items: center; text-align: center; color: #94a3b8; font-size: 11px; margin: 10px 0; text-transform: lowercase; }
...
div[data-testid="stTextInput"] { margin-bottom: -10px !important; }
    ...
    with col1:
        ...
        st.markdown("<p style='text-align: center; color: #475569; font-size: 13.5px; margin-bottom: 12px;'>Faça login ou experimente grátis agora mesmo!</p>", unsafe_allow_html=True)
        
        # Botão do Google que força a navegação na MESMA ABA
        components.html(f"""
            <button onclick="window.top.location.href='{oauth_url}';" class="google-btn-custom">
