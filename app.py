...
# ----------------------------------------------------
# 5. Fluxo de Autenticação Seguro (Atualizado com Layout Split e OAuth)
# ----------------------------------------------------
...
if not st.session_state.user_session:
    # 1. HACK AVANÇADO DE DOM: Fura o bloqueio do iframe do Streamlit para ler o "#" da URL vindo da Vercel
    components.html("""
        <script>
        try {
...
def exibir_tela_autenticacao():
    # Gera a URL de OAuth direta para o botão Google usando a variável do seu sistema
    oauth_url = f"{SUPABASE_URL}/auth/v1/authorize?provider=google&redirect_to=https://jurisprimeai.streamlit.app/"

    # CSS específico para limpar a tela e montar o design Split-Screen idêntico à referência
    st.markdown(f"""
    <style>
    .block-container {{
    ...
    /* Formatações do Form de E-mail para igualar ao botão cinza da concorrência */
    div[data-testid="stForm"] {{
        border: none !important;
        padding: 0 !important;
        background: transparent !important;
    ...
    with col1:
        ...
        # Botão HTML do Google
        st.markdown(f"""
            <a href="{oauth_url}" target="_self" class="google-btn-link">
    ...
    with col2:
        ...
        <div class="auth-right-panel">
            ...
            <h2 style="text-align: center; font-size: 40px; font-weight: 800; line-height: 1.2; margin-bottom: 40px; position: relative; z-index: 10;">
                A infraestrutura<br>definitiva para<br><span style="color: #38BDF8;">advogados de elite</span>
            </h2>
            ...
            <div style="background: rgba(255, 255, 255, 0.03); border: 1px solid rgba(255, 255, 255, 0.1); border-radius: 16px; padding: 32px; position: relative; z-index: 10; backdrop-filter: blur(10px);">
                <div style="color: #38BDF8; font-size: 40px; font-family: Georgia, serif; line-height: 0; margin-bottom: -5px; opacity: 0.8;">"</div>
                <p style="font-size: 14.5px; line-height: 1.6; color: #cbd5e1; margin-bottom: 24px; text-align: justify;">
                    A Ameno IA revolucionou a forma como conduzimos nosso trabalho no escritório. Com a capacidade de extrair fatos de reuniões com o AtaPrime e emitir pareceres detalhados no JusPrime, conseguimos otimizar nosso tempo e blindar nosso faturamento.
                </p>
                <div style="display: flex; align-items: center; gap: 14px;">
                    ...
                    <div>
                        <div style="font-weight: 700; font-size: 14px; color: white;">Mariana Costa</div>
                        <div style="font-size: 12px; color: #94a3b8;">Advogada Sênior</div>
                    </div>
                </div>
            </div>
        </div>
