...
# ----------------------------------------------------
# 5. Fluxo de Autenticação Seguro (Sanitizado)
# ----------------------------------------------------
...
if not st.session_state.user_session:
    # Captura o token vindo da URL da Landing Page
    components.html("""
        <script>
        try {
...
def exibir_tela_autenticacao():
    ...
    st.markdown("""<style>
.block-container { max-width: 1400px !important; padding-top: 2rem !important; padding-bottom: 0 !important; }
.stApp { background-color: #ffffff !important; }
.auth-right-panel { background: linear-gradient(135deg, #0B132B 0%, #0F172A 100%); border-radius: 24px; padding: 60px 40px; color: white; height: 85vh; display: flex; flex-direction: column; justify-content: center; box-shadow: 0 25px 50px -12px rgba(0, 0, 0, 0.25); position: relative; overflow: hidden; }
...
.google-btn-link { display: flex; align-items: center; justify-content: center; gap: 12px; width: 100%; border: 1px solid #e2e8f0; border-radius: 8px; padding: 12px; font-size: 15px; font-weight: 600; color: #1e293b !important; background: #ffffff; cursor: pointer; text-decoration: none; transition: all 0.2s; margin-bottom: 24px; box-shadow: 0 1px 2px rgba(0,0,0,0.05); }
.google-btn-link:hover { background: #f8fafc; border-color: #cbd5e1; }
.auth-divider { display: flex; align-items: center; text-align: center; color: #94a3b8; font-size: 12px; margin: 24px 0; text-transform: lowercase; }
...
div[data-testid="stForm"] button[kind="primary"] { background-color: #64748b !important; border-color: #64748b !important; color: white !important; width: 100% !important; border-radius: 8px !important; font-weight: 600 !important; height: 48px !important; margin-top: 8px !important; }
div[data-testid="stForm"] button[kind="primary"]:hover { background-color: #475569 !important; border-color: #475569 !important; }
div[data-testid="stTextInput"] input { background-color: #ffffff !important; border: 1px solid #cbd5e1 !important; color: #1e293b !important; }
</style>""", unsafe_allow_html=True)
    
    col1, espaco, col2 = st.columns([1.1, 0.2, 1.2])
    
    with col1:
        st.markdown("<br><br>", unsafe_allow_html=True)
        st.markdown("<h1 style='text-align: center; color: #0f172a; font-weight: 800; font-size: 34px; line-height: 1.2; margin-bottom: 16px;'>Sua rotina jurídica<br>mais eficiente</h1>", unsafe_allow_html=True)
        st.markdown("<p style='text-align: center; color: #475569; font-size: 15px; margin-bottom: 40px;'>Faça login ou experimente grátis<br>agora mesmo!</p>", unsafe_allow_html=True)
        
        st.markdown(f'<a href="{oauth_url}" target="_self" class="google-btn-link"><svg width="20" height="20" viewBox="0 0 24 24"><path d="M22.56 12.25c0-.78-.07-1.53-.2-2.25H12v4.26h5.92c-.26 1.37-1.04 2.53-2.21 3.31v2.77h3.57c2.08-1.92 3.28-4.74 3.28-8.09z" fill="#4285F4"/><path d="M12 23c2.97 0 5.46-.98 7.28-2.66l-3.57-2.77c-.98.66-2.23 1.06-3.71 1.06-2.86 0-5.29-1.93-6.16-4.53H2.18v2.84C3.99 20.53 7.7 23 12 23z" fill="#34A853"/><path d="M5.84 14.09c-.22-.66-.35-1.36-.35-2.09s.13-1.43.35-2.09V7.07H2.18C1.43 8.55 1 10.22 1 12s.43 3.45 1.18 4.93l2.85-2.22.81-.62z" fill="#FBBC05"/><path d="M12 5.38c1.62 0 3.06.56 4.21 1.64l3.15-3.15C17.45 2.09 14.97 1 12 1 7.7 1 3.99 3.47 2.18 7.07l3.66 2.84c.87-2.6 3.3-4.53 6.16-4.53z" fill="#EA4335"/></svg>Acessar com o Google</a>', unsafe_allow_html=True)
        ...
        st.markdown("<p style='text-align: center; font-size: 11px; color: #64748b; margin-top: 32px;'>Ao fazer login você concorda com os<br><strong>Termos de Uso</strong> e a <strong>Política de Privacidade</strong>.</p>", unsafe_allow_html=True)

    with col2:
        right_panel_html = (
            '<div class="auth-right-panel">'
            '<div style="display: flex; align-items: center; justify-content: center; gap: 10px; margin-bottom: 50px; position: relative; z-index: 10;">'
            '<svg style="width: 32px; height: 32px; color: #38BDF8;" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M19 11H5m14 0a2 2 0 012 2v6a2 2 0 01-2 2H5a2 2 0 01-2-2v-6a2 2 0 012-2m14 0V9a2 2 0 00-2-2M5 11V9a2 2 0 002-2m0 0V5a2 2 0 012-2h6a2 2 0 012 2v2M7 7h10"></path></svg>'
            '<span style="font-size: 28px; font-weight: 800; color: white; letter-spacing: -0.5px;">AMENO<span style="color: #38BDF8;">.IA</span></span>'
            '</div>'
            '<h2 style="text-align: center; font-size: 38px; font-weight: 800; line-height: 1.2; margin-bottom: 40px; position: relative; z-index: 10;">A infraestrutura<br>definitiva para<br><span style="color: #38BDF8;">advogados de elite</span></h2>'
            '<div style="background: rgba(255, 255, 255, 0.03); border: 1px solid rgba(255, 255, 255, 0.1); border-radius: 16px; padding: 32px; position: relative; z-index: 10; backdrop-filter: blur(10px);">'
            '<div style="color: #38BDF8; font-size: 40px; font-family: Georgia, serif; line-height: 0; margin-bottom: 15px; opacity: 0.8;">"</div>'
            '<p style="font-size: 14.5px; line-height: 1.6; color: #cbd5e1; margin-bottom: 24px; text-align: justify;">A Ameno IA revolucionou a forma como conduzimos nosso trabalho no escritório. Com a capacidade de extrair fatos de reuniões com o AtaPrime e emitir pareceres detalhados no JusPrime, conseguimos otimizar nosso tempo e blindar nosso faturamento.</p>'
            '<div style="display: flex; align-items: center; gap: 14px;">'
            '<div style="width: 44px; height: 44px; border-radius: 50%; background: #38BDF8; color: #0B132B; display: flex; align-items: center; justify-content: center; font-weight: 800; font-size: 16px;">MC</div>'
            '<div><div style="font-weight: 700; font-size: 14px; color: white;">Mariana Costa</div><div style="font-size: 12px; color: #94a3b8;">Advogada Sênior</div></div>'
            '</div>'
            '</div>'
            '</div>'
        )
